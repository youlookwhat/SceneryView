 ...
 * 点滴之行,看世界
 **/
public class CircleWaveProgressView2 extends View {
    ...
    public CircleWaveProgressView2(Context context) {
        this(context, null);
    }

    public CircleWaveProgressView2(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircleWaveProgressView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        waveLength = typedArray.getDimension(R.styleable.CircleWaveProgressView_wave_length, DensityUtil.dip2px(context, 20));
        //获取波浪高度
        waveHeight = typedArray.getDimension(R.styleable.CircleWaveProgressView_wave_height, DensityUtil.dip2px(context, 5));
        //获取波浪颜色
        wave_color = typedArray.getColor(R.styleable.CircleWaveProgressView_wave_color, Color.parseColor("#B76EFF"));
        //圆形背景颜色
        circle_bgcolor = typedArray.getColor(R.styleable.CircleWaveProgressView_circlebg_color, Color.WHITE);
        //当前进度
        currentProgress = typedArray.getFloat(R.styleable.CircleWaveProgressView_currentProgress, 10);
        //最大进度
        maxProgress = typedArray.getFloat(R.styleable.CircleWaveProgressView_maxProgress, 100);
        //第二层波浪的颜色
        second_WaveColor = typedArray.getColor(R.styleable.CircleWaveProgressView_second_color, Color.parseColor("#DEBCFF"));
        //记得把TypedArray回收
    ...
    private void init(Context context) {
        //设置自定义View的宽高
        waveDefaultSize = DensityUtil.dip2px(context, 152);
        waveDefaultHeight = DensityUtil.dip2px(context, 40);
        //设置自定义View的最大宽高
        waveMaxHeight = DensityUtil.dip2px(context, 152);
...
//        maxProgress = 100;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.d("ssd", getWidth() + "");
        //这里用到了缓存 根据参数创建新位图
        circleBitmap = Bitmap.createBitmap(waveActualSize, waveActualSizeHeight, Bitmap.Config.ARGB_8888);
        //以该bitmap为低创建一块画布
        bitmapCanvas = new Canvas(circleBitmap);
        //绘制圆形 半径设小了一点，就是为了能让波浪填充完整个圆形背景
//        bitmapCanvas.drawCircle(waveActualSize / 2, waveActualSize / 2, waveActualSize / 2 - DensityUtil.dip2px(getContext(), 8), circlePaint);
        RectF oval3 = new RectF(0, 0, waveActualSize, waveActualSizeHeight);// 设置个新的长方形
        bitmapCanvas.drawRoundRect(oval3, DensityUtil.dip2px(getContext(), 27), DensityUtil.dip2px(getContext(), 27), circlePaint);
        //绘制波浪形
...
//          canvas.drawPath(paintWavePath(),wavePaint);
        ...
        int i = DensityUtil.dip2px(getContext(), 1);
        paint.setStrokeWidth(i);
        paint.setStyle(Paint.Style.STROKE);
        RectF oval4 = new RectF(0.5f * i, 0.5f * i, waveActualSize - 0.5f * i, waveActualSizeHeight - 0.5f * i);// 设置个新的长方形
        canvas.drawRoundRect(oval4, DensityUtil.dip2px(getContext(), 27), DensityUtil.dip2px(getContext(), 27), paint);
    ...
    private Path paintWavePath() {
        ...
        wavePath.moveTo((currentPercent) * waveActualSize, -moveDistance);
...
//            wavePath.rQuadTo(waveLength / 2,-waveHeight,waveLength,0);
        ...
        //连接p1 - p2
        wavePath.lineTo(waveActualSize, waveActualSizeHeight);
        //连接p2 - p3
...
//                    waveProgressAnimat.setDuration(7000);
    ...
    private Path cavasSecondPath() {
        ...
        //移动到右上方，也就是p1点
        wavePath.moveTo((currentPercent) * waveActualSize, waveActualSizeHeight + moveDistance);
        //p1 - p0
        for (int i = 0; i < waveNumber * 2; i++) {
            wavePath.rQuadTo(secondWaveHeight, -waveLength / 2, 0, -waveLength);
            wavePath.rQuadTo(-secondWaveHeight, -waveLength / 2, 0, -waveLength);
        }
        //p3-p0的高度随着进度变化而变化
        wavePath.lineTo(0, 0);
        //连接p3 - p2
        wavePath.lineTo(0, waveActualSizeHeight);
        wavePath.lineTo(waveActualSizeHeight, waveActualSize);
        //连接p2 - p1
        wavePath.lineTo(waveActualSize, waveActualSizeHeight + moveDistance);
        //封闭起来填充
        wavePath.close();
        return wavePath;
